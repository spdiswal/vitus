...
describe("given a project of 4 files with a mix of passed and skipped tests", () => {
	const initialProject = dummyProject({
		files: [
			dummyFile("15b021ef72", {
				tests: [
					dummyTest("15b021ef72_50", { status: "passed" }),
			...
			dummyFile("a3fdd8b6c3", {
				tests: [
					...
					dummyTest("a3fdd8b6c3_52", { status: "skipped" }),
				],
			}),
	...
	`(
		"when a top-level test with id $path.1 has failed",
		(props: {
			path: TestPath
			duration: Duration
			...
			beforeEach(() => {
				actualProject = applyEvent(
					initialProject,
					testFailedEvent({ path: props.path, duration: props.duration }),
				)
	...
	describe("when a non-existing test has failed", () => {
		let actualProject: Project

		beforeEach(() => {
			actualProject = applyEvent(
				initialProject,
				testFailedEvent({ path: ["15b021ef72", "15b021ef72_8"], duration: 2 }),
			)
		})
	...
	describe("when a test in a non-existing file has failed", () => {
		let actualProject: Project

		beforeEach(() => {
			actualProject = applyEvent(
				initialProject,
				testFailedEvent({ path: ["f9bb9e8bc0", "f9bb9e8bc0_0"], duration: 2 }),
			)
		})
